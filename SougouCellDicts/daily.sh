...
while [ "x${DICT_IDS[i]}" != "x" ]; do
  ...
  python ./scel2txt.py ./out/scel/$id.scel ./out/scel/$id.txt
  txt=$(cat out/scel/$id.txt)
  header="---\nname: ${DICT_PREFIX}.${shortname}\nversion: \"${date}\"\nsort: by_weight\nuse_preset_vocabulary: true\n...\n\n"
  echo -e "$header${txt}" > out/rime/${FILE_PREFIX}.${name}.dict.yaml
  i=$(( $i + 1 ))
done
...
# $GNU_SED '/网络流行新词【官方推荐】/d' -i  $EXT_PATH
